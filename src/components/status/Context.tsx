...
export const Context: React.FC<ContextProps> = ({ children }) => {
    ...
    React.useEffect(() => {
        ...
        charactorDispatch({ type: "UPDATE_STATUS", statuses, equiomentOptions: options, skillOptions: skillContext });

    }, [equipments]);
    React.useEffect(() => {
        const options = reduceSkills(appliedSkills);
        setSkillContext(options);
        charactorDispatch({ type: "UPDATE_STATUS", statuses, equiomentOptions: optionContext, skillOptions: options });
    }, [appliedSkills]);
    React.useEffect(() => {
        charactorDispatch({ type: "UPDATE_STATUS", statuses, equiomentOptions: optionContext, skillOptions: skillContext });
    }, [statuses]);
